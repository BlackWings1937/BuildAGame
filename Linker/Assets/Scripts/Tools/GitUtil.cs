...
{
    public static string STR_NAME_GIT_PROJ_NAME = "LinkerData";
    ...
    public static bool CheckGitWorkTreeCleanAtFloder(string path) {
        var cachePath = GetGitCacheAreaPath(path);
        Win32Util.RunCmd("cd "+cachePath);
        var result = Win32Util.RunCmd("git status");
        if (result.Contains("working tree clean")) {
            return true;
        }
        return false;
    }
