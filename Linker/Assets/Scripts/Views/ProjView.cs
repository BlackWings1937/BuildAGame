...
public class ProjView : BaseView {
    ...
    public override void UpdateView(Dictionary<string, object> info)
    {
        ...
        var contentHeight =Mathf.Max(myGroup_.padding.top + myGroup_.padding.bottom + (childCount - 1) * myGroup_.spacing,0);
        ...
        foreach (var pair in info) {
            ...
            var cellData = new CellOldProjsData();
            //cellData.myProjName_ = dicInfo[""];
            //cell.GetComponent<CellOldProjs>().UpdateData()
        }
    }

    override protected void registerViewEvent(){
		transform.Find(BTN_N_START_NEW).GetComponent<Button>().onClick.AddListener(()=>{ Debug.Log("start new"); });
		transform.Find(BTN_N_START_OLD).GetComponent<Button>().onClick.AddListener(()=>{ Debug.Log("start old"); });
	}
