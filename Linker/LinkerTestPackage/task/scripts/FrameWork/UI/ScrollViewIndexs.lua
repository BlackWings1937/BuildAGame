...
local CellIndex = requirePack("scripts.FrameWork.UI.CellIndex");

local ScrollViewIndexs = class("ScrollViewIndexs",function()
    return cc.Node:create(); 
end);
ScrollViewIndexs.EventType = {
}
g_tConfigTable.CREATE_NEW(ScrollViewIndexs);

function ScrollViewIndexs:ctor()
...
function ScrollViewIndexs:SetEnable(v)
    self.sw_:setVisible(v);
    self.slider_:setVisible(v);
end
function ScrollViewIndexs:UpdateByList(list)
    self.content_:RemoveAllCell();
    local count = #list;
    for i = 1,count,1 do 
        local cell = CellIndex.new();
